...
def update_dns_record(record_name, ip, current_content):
    ...
    try:
        ...
        if result["status"] == "SUCCESS":
            print(f"DNS record updated successfully for {record_name or '@'}.")
        else:
            print(f"Failed to update DNS record for {record_name or '@'}: {result.get('message', 'Unknown error')}")
        return result
    except requests.exceptions.RequestException as e:
        print(f"Error occurred while updating DNS record for {record_name or '@'}: {str(e)}")
        return {"status": "ERROR", "message": str(e)}
    except json.JSONDecodeError:
        print(f"Failed to decode JSON response from API for {record_name or '@'}.")
...
if __name__ == "__main__":
    ...
    if current_records:
        ...
        print("Failed to retrieve current DNS records.")
